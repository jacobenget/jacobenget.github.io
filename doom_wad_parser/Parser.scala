...
class ByteReader(val bytes: Array[Byte], override val offset: Int) extends Reader[Byte] {
  ...
  override def source = ArrayCharSequence(bytes drop(offset) map (_.toChar))
}

object DoomWadParser extends Parsers {
  type Elem = Byte
  ...
  /**
    * The main entrypoint of this object that parses an entire Doom 1 WAD file
    *
    * @param byteReader - allows reading of the bytes of the Doom 1 WAD 
    * @return the result of parsing the Doom 1 WAD
    */
  def parseDoomWad(byteReader: ByteReader): ParseResult[DoomWad] = (parseLumps >> { _ match { case (wadType, lumps) => {
